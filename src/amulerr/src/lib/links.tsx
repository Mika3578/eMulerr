...
export function normalizeEd2kHash(value: string | null | undefined): string | null {
  if (!value?.trim()) {
    return null
  }
...
export function fromQbittorrentHashStrict(qbittorrentHash: string): string | null {
...
export function fromEd2kLink(ed2kLink: string) {
  const match =
    /^ed2k:\/\/\|file\|(?<name>[^|]+)\|(?<size>\d+)\|(?<hash>[0-9a-fA-F]{32})\|\/?$/.exec(ed2kLink)
