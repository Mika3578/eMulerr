      ...
      POST: async ({ request }) => {
        ...
        const addResult = await useAmule(async (amule) => {
          const isPresent = async () => {
            ...
            return [...downloads, ...shared].some((f) => sameEd2kHash(f.fileHash, hash))
          }
          ...
          if (!categoryId) {
            return { ok: false as const, error: `Category ${category} not found`, status: 404 }
          }

          if (!await amule.addEd2kLink(ed2kLink, categoryId) && !(await isPresent())) {
            return { ok: false as const, error: `Failed to add torrent`, status: 500 }
          }
